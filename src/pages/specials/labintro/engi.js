...
const LabIntroEngiPage = ({ data }) => {
  const { edges: contents } = data.allLabIntroEngiJson;
  return (
    <Layout>
      <HelmetForOGP
        title={"名大の研究室紹介 - 工学部"}
...
const LabIntroEngiPageQuery = () => (
  <StaticQuery
    query={graphql`
      query LabIntroEngiPageQuery {
        allLabIntroEngiJson {
          edges {
            node {
              video
    ...
    `}
    render={(data) => <LabIntroEngiPage data={data} />}
  />
);

export default LabIntroEngiPageQuery;
