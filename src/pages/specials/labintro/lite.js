...
const LabIntroLitePage = ({ data }) => {
  const { edges: contents } = data.allLabIntroLiteJson;
  return (
    <Layout>
      <HelmetForOGP
        title={"名大の研究室紹介 - 文学部"}
...
const LabIntroLitePageQuery = () => (
  <StaticQuery
    query={graphql`
      query LabIntroLitePageQuery {
        allLabIntroLiteJson {
          edges {
            node {
              video
    ...
    `}
    render={(data) => <LabIntroLitePage data={data} />}
  />
);

export default LabIntroLitePageQuery;
