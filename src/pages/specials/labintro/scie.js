...
const LabIntroSciePage = ({ data }) => {
  const { edges: contents } = data.allLabIntroScieJson;
  return (
    <Layout>
      <HelmetForOGP
        title={"名大の研究室紹介 - 理学部"}
...
const LabIntroSciePageQuery = () => (
  <StaticQuery
    query={graphql`
      query LabIntroSciePageQuery {
        allLabIntroScieJson {
          edges {
            node {
              video
    ...
    `}
    render={(data) => <LabIntroSciePage data={data} />}
  />
);

export default LabIntroSciePageQuery;
