...
const LabIntroAgriPage = ({ data }) => {
  const { edges: contents } = data.allLabIntroAgriJson;
  return (
    <Layout>
      <HelmetForOGP
        title={"名大の研究室紹介 - 農学部"}
...
const LabIntroAgriPageQuery = () => (
  <StaticQuery
    query={graphql`
      query LabIntroAgriPageQuery {
        allLabIntroAgriJson {
          edges {
            node {
              video
    ...
    `}
    render={(data) => <LabIntroAgriPage data={data} />}
  />
);

export default LabIntroAgriPageQuery;
